...
### Monorepo Structure
Three npm workspaces: `packages/shared`, `frontend`, `backend`. Backend is CommonJS, frontend is ESM.
...
- Path aliases resolved at runtime via `tsconfig-paths/register` (dev) and compiled paths (build uses `tsconfig.build.json`)
...
- `@shared` → `packages/shared/src` (Vite config)
